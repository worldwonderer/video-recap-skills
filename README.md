...
第一条命令仅依赖现有的 Python 标准库 + ffmpeg，会生成 `.subtitle_measure/preview/` 网格红框预览与 `subtitle_positions.json`；不加 `--accept-detected` 可查看预览后手动确认坐标。当前测量坐标仅支持方形像素（SAR `1:1`）视频，避免将滤镜像素与 ASS 显示坐标混用。遮罩可用 `SUBTITLE_MASK_OPACITY`（`0..1`）和 `SOURCE_SUBTITLE_MASK_TIMING=all|narration` 调整。
