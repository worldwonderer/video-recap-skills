...
| `asr_segment_seconds` | 30 / `ASR_SEGMENT_SECONDS` | ASR 分段窗口秒数；越小对白时间戳越精细，代价是更多 ASR 调用 |
...
视频组装时根据 `ducking_mode` 和解说段的 `overlaps_speech` 标记，用 ffmpeg volume 表达式逐段构建原声音量包络（`_build_audio_filter_complex`）：

| 情况 | 原声音量 | 说明 |
|------|---------|------|
| 与对白重叠的 beat | `speech_ducking_volume` (0.2) | 大幅压低，让解说清晰 |
| 落在安静窗口的 beat | `zone_ducking_volume` (0.12) | 压低 |
| 无解说时段 | `ducking_orig_volume` (0.5) | 原声保留 |

当全部 beat 都落在安静窗口时，改用梯形淡入淡出（`zone_fade_seconds`）避免音量突变爆音。`--ducking sidechaincompress|none` 可切换为经典自动闪避或完全不闪避。
