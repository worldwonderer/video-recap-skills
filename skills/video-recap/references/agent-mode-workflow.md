...
1. `clip_plan.json`：选择要保留的原片片段，时间戳使用**原视频时间**。
2. `narration.json`：解说词也使用**原视频时间**，并且应落在已选择片段内。

默认不允许 `clip_plan.json` 片段重叠；如果确实需要重复使用同一段原片，先拆成不重叠片段；如确实需要重复画面，当前可在 `narration.json` 段落里加 `source_clip_id` 指向 `clip_plan_validated.json` 中的片段编号。
