...
def test_overview_becomes_primary_description_and_keeps_frame_facts(tmp_path):
    scenes = [
        {"scene_id": 0, "description": "frame desc 0", "frame_facts": {"1.0": ["a"]}, "depth_analysis": "d0"},
        {"scene_id": 1, "description": "frame desc 1", "frame_facts": {"2.0": ["b"]}},
    ]
    ov = tmp_path / "mimo_video_overview.json"
    ov.write_text(json.dumps({"chunks": [
        {"scene_id": 0, "content": "五竹抱着婴儿在竹林中奔逃，黑衣杀手围攻。"},
    ]}), encoding="utf-8")
...
def test_overview_merge_does_not_regress_substrate_grade(tmp_path):
    ...
    ov = tmp_path / "mimo_video_overview.json"
    ov.write_text(json.dumps({"chunks": [{"scene_id": 0, "content": "丰富的视频理解描述。"}]}), encoding="utf-8")
    after = assess_understanding_substrate(understand._merge_overview_into_scenes(scenes, ov), asr)["level"]
...
def test_overview_absent_or_rejected_is_noop(tmp_path):
    ...
    assert understand._merge_overview_into_scenes([dict(s) for s in scenes], tmp_path / "nope.json")[0]["description"] == "frame"
    # moderation-rejected chunk content -> not usable -> scene keeps frame description
    ov = tmp_path / "mimo_video_overview.json"
    ov.write_text(json.dumps({"chunks": [{"scene_id": 0, "content": "The request was rejected because it was considered high risk"}]}), encoding="utf-8")
