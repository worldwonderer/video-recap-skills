...
def _run_review_with_finding(monkeypatch, work_dir, finding):
    ...
    def fake_api(payload):
        ...
        return {"choices": [{"message": {"content": json.dumps(
            {"verdict": "REVISE", "summary": "s", "findings": [finding]}, ensure_ascii=False)}}]}
...
def test_review_payload_is_deterministic(monkeypatch, tmp_path):
    ...
    payloads = _run_review_with_finding(
        ...
        {"segment": 0, "severity": "warning", "category": "weak_hook", "issue": "i", "fix": "f"})
    payload = payloads[0]
...
def test_craft_error_is_clamped_and_does_not_gate(monkeypatch, tmp_path):
    ...
    NOT count as a gating error in recap._review_result_status."""
    _run_review_with_finding(
        monkeypatch, tmp_path,
        {"segment": 0, "severity": "error", "category": "weak_hook", "issue": "开头平淡", "fix": "加悬念"})

    review_json = json.loads((tmp_path / "narration_review.json").read_text(encoding="utf-8"))
...
def test_hallucination_error_still_gates(monkeypatch, tmp_path):
    ...
    _run_review_with_finding(
        monkeypatch, tmp_path,
        {"segment": 0, "severity": "error", "category": "hallucination", "issue": "凭空虚构", "fix": "删掉"})
...
def test_parse_clamps_craft_error_directly():
    ...
    r = review.parse_review_response(json.dumps({
        "verdict": "REVISE",
        "findings": [
